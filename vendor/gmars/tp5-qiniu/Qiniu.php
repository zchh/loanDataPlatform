...
//            throw new Exception('上传出错'.serialize($err));
     ...
     * 多文件上传
     */
    public function uploadAll($saveName = '', $bucket = '')
    {
        $token = $this->_getUploadToken($bucket);
        $files = $_FILES;
        if (empty($files)) {
        ...
        $uploadManager = new UploadManager();
        $result = [];
        for ($i=0;$i<count($values[0]['tmp_name']);$i++){
            $saveName = '';
            if (empty($saveName)) {
                $saveName = hash_file('sha1', $values[0]['tmp_name'][$i]).time();
            }
            $infoArr = explode('.', $values[0]['name'][$i]);
            $extension = array_pop($infoArr);
            $fileInfo = $saveName . '.' . $extension;
            list($ret, $err) = $uploadManager->putFile($token, $saveName, $values[0]['tmp_name'][$i]);
            if ($err !== null) {
//                throw new Exception('上传出错'.serialize($err));
                return false;
            }
